...
def _insert_lead(conn: psycopg.Connection, *, intake_id: str, request_id: str, received_at_utc: str, lead_source: str, payload: Dict[str, Any]) -> None:
    ...
    # Always store the raw payload in the chosen JSON/JSONB column.
    # IMPORTANT: psycopg3 will not adapt a plain dict unless wrapped.
    # Use Json/Jsonb wrappers so parameter adaptation is deterministic.
    if cols.get(json_col) == "jsonb":
        record[json_col] = Jsonb(payload)
    else:
